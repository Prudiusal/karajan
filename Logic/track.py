...
class Track:
        ...
        for processor_conf in self.plugins_data:
            ...
            if not self.proc_funcs.get(processor_type):
                raise WrongDawDreamerProcessor(
                    "Type should match the " "processors"
                )
            ...
            if new_proc:  # error during creation may occur, so will be False
                ...
                if not len(self.processors):
                    ...
                    logger_track.error(
                        f"Track {self.track_name} has no synth"
            ...
            if (
                "ad2" in processor_name.lower()
            ):  # this plugin requires summation
                logger_track.debug("Summatıon for Addictive Drums 2 is used")
                adder_func = self.proc_funcs.get("add")
                self.processors["ad2_adder"] = adder_func(
                    "ad2_adder", 18 * [1]
                )
            elif (
                "kontakt" in processor_name.lower()
            ):  # this plugin requires summation
                logger_track.debug("Summation for KONTAKT is used")
                adder_func = self.proc_funcs.get("add")
                self.processors["kontakt_adder"] = adder_func(
                    "kontakt_adder", 64 * [1]
                )
            ...
            if processor_conf.get("sideChain"):
                ...
                self.sidechains[processor_name] = processor_conf.get(
                    "sideChain"
                )
        # to use by sidechain.
    ...
    def check_inputs(proc_names_list):
        for proc_name in proc_names_list:
            assert type(proc_name) is str, "Should be string"
