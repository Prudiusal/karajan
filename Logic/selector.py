...
class Selector:
    ...
    def __call__(self, config):
        ...
        if original_number_of_tracks == 1:
            raise StemsNotDeletedError(f'{config.get("Name")} -'
                                       f'{config["Artist"]} has only 1 not '
                                       f'empty stem')

        num_tracks_to_remove = math.floor(len(config['Tracks']) *
                                          self.k_remove)
        ...
        if original_number_of_tracks == final_number_of_tracks:
            raise UnableToDeleteStemsError(f'{config.get("Name")} no deleted '
                                           f'stems, Skipped')
        return config
    ...
    def clear_names(self, config, names_to_delete=None):
        ...
        for i, track in enumerate(config.get('Tracks')):
            ...
            if any([name.lower() in track_name for name in
                    names_to_delete]):
                ids_remove.append(i)
        ...
        for i in sorted(ids_remove, reverse=True):
            if not self.track_is_crutial(config, i):
                ...
                logger_sel.warning(f'{config.get("Tracks")[i]["stem_name"]} '
                                   'is crucial! SKIPPED')
